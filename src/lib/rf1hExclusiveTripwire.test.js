// docs/known-risks.md item 78 — tripwire de segurança, mesmo padrão do
// rf1hUncondTripwire.test.js: pineConfig.rf1hExclusiveEnabled tem que existir
// SÓ em scripts/backtestPineConfig.js. Se essa chave algum dia aparecer como
// uma entrada de DEFAULTS/SYNCED_STRATEGY_KEYS em src/lib/pineParser.js
...
// antes de virar um toggle de produção sem gate de revisão de código (essa
// flag desliga a cascata nativa por completo, não é um detalhe menor).
//
...
// (`rf1hExclusiveEnabled: false,`), não com a menção em prosa dentro de um
// comentário explicativo (sem `:` logo em seguida) que os 3 arquivos já
// têm de propósito.
const KEY_AS_OBJECT_ENTRY = /rf1hExclusiveEnabled\s*:/;

describe('rf1hExclusiveEnabled — tripwire de isolamento backtest-only', () => {
  ...
  // DEFAULTS. Bastaria adicionar 'rf1hExclusiveEnabled' ao array (sem tocar DEFAULTS
  ...
  it('NÃO está em nenhuma lista de sync (SYNCED_STRATEGY_KEYS) dos dois arquivos de produção', () => {
    for (const rel of ['./pineParser.js', '../../scripts/adminPineConfig.js']) {
      ...
      expect(source, `${rel} não pode citar a chave como string de sync`)
        .not.toMatch(/'rf1hExclusiveEnabled'/);
    }
  });
});
