// docs/known-risks.md item 102 — tripwire de segurança, mesmo padrão do
// buyRegimeFilterTripwire.test.js: pineConfig.rfStructuralStopEnabled tem
// que existir SÓ em scripts/backtestPineConfig.js. Se essa chave algum dia
...
// alto e cedo, antes de virar um toggle de produção (mudança de ESTRATÉGIA,
// não detalhe de mecanismo) sem gate de revisão de código.
//
...
// (`rfStructuralStopEnabled: false,`), não com a menção em prosa dentro de
// um comentário explicativo (sem `:` logo em seguida) que os 3 arquivos já
// têm de propósito.
const KEY_AS_OBJECT_ENTRY = /rfStructuralStopEnabled\s*:/;

describe('rfStructuralStopEnabled — tripwire de isolamento backtest-only', () => {
  ...
  // DEFAULTS. Bastaria adicionar 'rfStructuralStopEnabled' ao array (sem tocar DEFAULTS
  ...
  it('NÃO está em nenhuma lista de sync (SYNCED_STRATEGY_KEYS) dos dois arquivos de produção', () => {
    for (const rel of ['./pineParser.js', '../../scripts/adminPineConfig.js']) {
      ...
      expect(source, `${rel} não pode citar a chave como string de sync`)
        .not.toMatch(/'rfStructuralStopEnabled'/);
    }
  });
});
