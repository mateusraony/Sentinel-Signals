// docs/known-risks.md item 100 — tripwire de segurança, mesmo padrão do
// allowedSideTripwire.test.js: pineConfig.buyRegimeFilterEnabled tem que
...
// (`buyRegimeFilterEnabled: false,`), não com a menção em prosa dentro de um
// comentário explicativo (sem `:` logo em seguida) que os 3 arquivos já
// têm de propósito.
const KEY_AS_OBJECT_ENTRY = /buyRegimeFilterEnabled\s*:/;

describe('buyRegimeFilterEnabled — tripwire de isolamento backtest-only', () => {
  ...
  // DEFAULTS. Bastaria adicionar 'buyRegimeFilterEnabled' ao array (sem tocar DEFAULTS
  ...
  it('NÃO está em nenhuma lista de sync (SYNCED_STRATEGY_KEYS) dos dois arquivos de produção', () => {
    for (const rel of ['./pineParser.js', '../../scripts/adminPineConfig.js']) {
      ...
      expect(source, `${rel} não pode citar a chave como string de sync`)
        .not.toMatch(/'buyRegimeFilterEnabled'/);
    }
  });
});
