// Cliente HTTP para o backend Postgres/Neon (Fase 6/10 do plano de migração
// Firestore→Neon, /root/.claude/plans/baseando-nos-dados-que-partitioned-pixel.md).
// Preserva **exatamente** a forma externa de `backend` que o antigo adaptador
// Firestore (agora preservado como src/api/entitiesFirestoreLegacy.js —
// referência de rollback) sempre teve (entities.<Nome>.{list,filter,get,set,
// create,createUnique,update,delete,bulkCreate,deleteMany}, locks, tradeOps,
// quota) — os consumidores não mudaram nenhuma outra linha no dia do cutover.
//
