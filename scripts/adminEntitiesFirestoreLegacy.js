// Firestore admin adapter — RENAMED here (Fase 10 do plano de migração
// Firestore→Neon, /root/.claude/plans/baseando-nos-dados-que-partitioned-pixel.md)
// from scripts/adminEntities.js, which is now a thin Postgres re-export of
// db/pgEntitiesCore.mjs (build-scan.mjs/build-backfill.mjs keep redirecting
// '@/api/entities' there unchanged — the live scanner cron moves to
// Postgres). This file stays exactly as it was, Firestore-backed, because
// 4 scripts still need to talk to the REAL Firestore regardless of cutover
// status: scripts/backup-firestore.mjs and backfill-rtdb.mjs (both keep
// running until Firestore is decommissioned, fase 11) and
// scripts/migrate-firestore-to-postgres.mjs/verify-postgres-migration.mjs
// (whose entire job IS reading Firestore). Not dead code — do not delete
// without checking those 4 importers first.
//
// Original header, still accurate for what this file does: Node/GitHub
// Actions counterpart to src/api/entities.js — same
